...
export default function StoreCheckboxes({
  selected,
  onChange,
}: StoreCheckboxesProps) {
  const toggleStore = (store: string) => {
    const newSelected = selected.includes(store)
      ? selected.filter((s) => s !== store)
      : [...selected, store];
          ...
          <li key={store}>
            ...
            >
              <h4 className="text-lg text-black-1 font-medium font-figtree">
                {SUPERMARKET_DISPLAY_NAMES[store]}
              </h4>
              <div
                className={`${
                  checked
                ...
                } inline-flex w-[22px] h-[22px] rounded-md items-center justify-center`}
              >
                {checked && (
                  <svg
                    width={12}
